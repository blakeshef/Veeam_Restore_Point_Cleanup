# ============================================================
# Veeam Stale Restore-Point Report + Cleanup — VMware Only
# ============================================================
...
$CsvPath              = 'C:\Temp\Veeam-StaleVMwareVMs.csv'
...
# ✅ VMware retrieval — collects from all available methods (parent + children)
...
# ✅ VMware detection by Platform string + VmCount filter
...
if ($EnableDiag) {
    Write-Host ""
    Write-Host "[DIAG] VMware backups detected: $($vmwareBackups.Count)" -ForegroundColor Magenta
}
...
if ($vmwareBackups.Count -eq 0) {
    ...
    Write-Host ""
    Write-Host "$staleCount stale VMware VM(s) found." -ForegroundColor Green
    Write-Host "Report written to $CsvPath" -ForegroundColor Green

} else {

    Write-Host ""
    Write-Host "VMware jobs exist, but no stale restore points were found before the cutoff date." -ForegroundColor Green
    Write-Host ""
...
if ($staleCount -eq 0) {
    ...
    # ============================================================
    #  Interactive VMware Restore-Point Cleanup (CSV-based)
    # ============================================================
    ...
    while ($true) {
        ...
        Write-Host "  VMware Stale Restore-Point Cleanup" -ForegroundColor Cyan
        ...
        if ($selection -match '^(q|quit|exit)$') {
            Write-Host "Exiting VMware cleanup. Done." -ForegroundColor Green
            break
        }
        ...
        # Locate the VMware backup
        $backupObj = Get-VBRBackup | Where-Object {
            $_.Name -eq $target.BackupName -and (
                $_.BackupPlatform.Platform.ToString() -match '^EVmware$|VmWare'
            )
        } | Select-Object -First 1

        if (-not $backupObj) {
            Write-Warning "Could not re-locate VMware backup '$($target.BackupName)'."
            continue
        }

        $allPoints = $null
        try { $allPoints = Get-VMwareRestorePoints -Backup $backupObj } catch { }
        ...
        Write-Host "`nThe following VMware restore points will be DELETED:" -ForegroundColor Yellow
        ...
        try {
            ...
            Write-Host "Updated VMware CSV: $CsvPathCleanup" -ForegroundColor DarkGray
