# ============================================================
# Veeam Stale Restore-Point Report + Cleanup — Hyper-V Only
# ============================================================
...
$CsvPath              = 'C:\Temp\Veeam-StaleHyperVVMs.csv'
...
# ✅ Hyper-V retrieval — collects from all available methods (parent + children)
...
# ✅ Hyper-V detection by Platform string + VmCount filter
...
if ($EnableDiag) {
    Write-Host ""
    Write-Host "[DIAG] Hyper-V backups detected: $($hypervBackups.Count)" -ForegroundColor Magenta
}
...
if ($hypervBackups.Count -eq 0) {
    ...
    Write-Host ""
    Write-Host "$staleCount stale Hyper-V VM(s) found." -ForegroundColor Green
    Write-Host "Report written to $CsvPath" -ForegroundColor Green

} else {

    Write-Host ""
    Write-Host "Hyper-V jobs exist, but no stale restore points were found before the cutoff date." -ForegroundColor Green
    Write-Host ""
...
if ($staleCount -eq 0) {
    ...
    # ============================================================
    #  Interactive Hyper-V Restore-Point Cleanup (CSV-based)
    # ============================================================
    ...
    while ($true) {
        ...
        Write-Host "  Hyper-V Stale Restore-Point Cleanup" -ForegroundColor Cyan
        ...
        if ($selection -match '^(q|quit|exit)$') {
            Write-Host "Exiting Hyper-V cleanup. Done." -ForegroundColor Green
            break
        }
        ...
        # Locate the Hyper-V backup
        $backupObj = Get-VBRBackup | Where-Object {
            $_.Name -eq $target.BackupName -and (
                $_.BackupPlatform.Platform.ToString() -match 'HyperV|EHyperV'
            )
        } | Select-Object -First 1

        if (-not $backupObj) {
            Write-Warning "Could not re-locate Hyper-V backup '$($target.BackupName)'."
            continue
        }

        $allPoints = $null
        try { $allPoints = Get-HyperVRestorePoints -Backup $backupObj } catch { }
        ...
        Write-Host "`nThe following Hyper-V restore points will be DELETED:" -ForegroundColor Yellow
        ...
        try {
            ...
            Write-Host "Updated Hyper-V CSV: $CsvPathCleanup" -ForegroundColor DarkGray
