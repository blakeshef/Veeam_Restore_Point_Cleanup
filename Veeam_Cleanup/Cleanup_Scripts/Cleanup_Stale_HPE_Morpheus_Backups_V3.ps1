# ============================================================
# Veeam Stale Restore-Point Report + Cleanup — HPE Morpheus
# ============================================================
...
$CsvPath              = 'C:\Temp\Veeam-StaleMorpheusVMs.csv'
...
# ✅ Robust HPE Morpheus retrieval — collects from all available methods
function Get-MorpheusRestorePoints {
    ...
    # Method 3: Direct COib enumeration
    try {
...
# ✅ Detect HPE Morpheus by TypeToString
...
if ($EnableDiag) {
    Write-Host ""
    Write-Host "[DIAG] HPE Morpheus backups detected: $($morpheusBackups.Count)" -ForegroundColor Magenta
}
...
if ($morpheusBackups.Count -eq 0) {
    ...
    Write-Host ""
    Write-Host "$staleCount stale HPE Morpheus VM(s) found." -ForegroundColor Green
    Write-Host "Report written to $CsvPath" -ForegroundColor Green

} else {

    Write-Host ""
    Write-Host "HPE Morpheus jobs exist, but no stale restore points were found before the cutoff date." -ForegroundColor Green
    Write-Host ""
...
if ($staleCount -eq 0) {
    ...
    # ============================================================
    #  Interactive HPE Morpheus Restore-Point Cleanup (CSV-based)
    # ============================================================
    ...
    while ($true) {
        ...
        Write-Host "  HPE Morpheus Stale Restore-Point Cleanup" -ForegroundColor Cyan
        Write-Host "============================================================" -ForegroundColor Cyan

        $tableText = $displayRows | Format-Table -AutoSize -Wrap | Out-String
        ...
        if ($selection -match '^(q|quit|exit)$') {
            Write-Host "Exiting HPE Morpheus cleanup. Done." -ForegroundColor Green
            break
        }
        ...
        # Locate the HPE Morpheus backup
        $backupObj = Get-VBRBackup | Where-Object {
            $_.Name -eq $target.BackupName -and ($_.TypeToString -match 'HPE Morpheus')
        } | Select-Object -First 1

        if (-not $backupObj) {
            Write-Warning "Could not re-locate HPE Morpheus backup '$($target.BackupName)'."
            continue
        }

        $allPoints = $null
        try { $allPoints = Get-MorpheusRestorePoints -Backup $backupObj } catch { }
        ...
        Write-Host "`nThe following HPE Morpheus restore points will be DELETED:" -ForegroundColor Yellow
        ...
        try {
            ...
            Write-Host "Updated HPE Morpheus CSV: $CsvPathCleanup" -ForegroundColor DarkGray
