# ============================================================
# Veeam Stale Restore-Point Report + Cleanup — Proxmox Only
# ============================================================
...
$CsvPath              = 'C:\Temp\Veeam-StaleProxmoxVMs.csv'
...
if ($proxmoxBackups.Count -eq 0) {
    ...
    Write-Host ""
    Write-Host "$staleCount stale Proxmox VM(s) found." -ForegroundColor Green
    Write-Host "Report written to $CsvPath" -ForegroundColor Green

} else {

    Write-Host ""
    Write-Host "Proxmox jobs exist, but no stale restore points were found before the cutoff date." -ForegroundColor Green
    Write-Host ""
...
if ($staleCount -eq 0) {
    ...
    # ============================================================
    #  Interactive Proxmox Restore-Point Cleanup (CSV-based)
    # ============================================================
    ...
    while ($true) {
        ...
        Write-Host "  Proxmox Stale Restore-Point Cleanup" -ForegroundColor Cyan
        ...
        if ($selection -match '^(q|quit|exit)$') {
            Write-Host "Exiting Proxmox cleanup. Done." -ForegroundColor Green
            break
        }
        ...
        # Locate the Proxmox backup
        $backupObj = Get-VBRBackup | Where-Object {
            $_.Name -eq $target.BackupName -and ($_.TypeToString -match 'Proxmox')
        } | Select-Object -First 1

        if (-not $backupObj) {
            Write-Warning "Could not re-locate Proxmox backup '$($target.BackupName)'."
            continue
        }

        $allPoints = $null
        try { $allPoints = Get-ProxmoxRestorePoints -Backup $backupObj } catch { }
        ...
        Write-Host "`nThe following Proxmox restore points will be DELETED:" -ForegroundColor Yellow
        ...
        try {
            ...
            Write-Host "Updated Proxmox CSV: $CsvPathCleanup" -ForegroundColor DarkGray
