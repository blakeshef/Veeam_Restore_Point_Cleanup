# ============================================================
# Veeam Stale Restore-Point Report + Cleanup
# Veeam Agent for Windows + Linux Only
# Detects:
...
$CsvPath              = 'C:\Temp\Veeam-StaleAgentBackups.csv'
...
# ✅ Agent retrieval — wildcard -Name "*" is required for Agent restore points
        ...
        if ($children) {
            ...
            foreach ($child in $children) {
                try {
                    ...
                    if ($childRp) { $rpList += $childRp }
                } catch { }
            }
            if ($rpList.Count -gt 0) { return $rpList }
        }
    } catch { }
...
# ✅ Detection rule — covers Windows + Linux Agents
...
if ($agentBackups.Count -eq 0) {
    ...
    Write-Host ""
    Write-Host "$staleCount stale Veeam Agent computer(s) found." -ForegroundColor Green
    Write-Host "Report written to $CsvPath" -ForegroundColor Green

} else {

    Write-Host ""
    Write-Host "Veeam Agent jobs exist, but no stale restore points were found before the cutoff date." -ForegroundColor Green
    Write-Host ""
...
if ($staleCount -eq 0) {
    ...
    # ============================================================
    #  Interactive Veeam Agent Restore-Point Cleanup (CSV-based)
    # ============================================================
    ...
    while ($true) {
        ...
        $displayRows = foreach ($r in ($cleanupResults | Sort-Object LatestRestorePoint, ComputerName)) {
            $cleanupMap["$rowNum"] = $r
        ...
        Write-Host "  Veeam Agent (Windows + Linux) Stale Restore-Point Cleanup" -ForegroundColor Cyan
        ...
        $selection = Read-Host "Enter the Row number to delete that computer's stale restore points (or 'Q' to quit)"

        if ([string]::IsNullOrWhiteSpace($selection)) { continue }

        if ($selection -match '^(q|quit|exit)$') {
            Write-Host "Exiting Veeam Agent cleanup. Done." -ForegroundColor Green
            break
        }
        ...
        # Locate the Agent backup
        $backupObj = Get-VBRBackup | Where-Object {
            $_.Name -eq $target.BackupName -and (
                ($_.TypeToString -match 'Windows Agent Backup|Linux Agent Backup') -or
                ($_.BackupPlatform.Platform.ToString() -match 'EEndPoint|ELinuxPhysical|EpAgentBackup')
            )
        } | Select-Object -First 1

        if (-not $backupObj) {
            Write-Warning "Could not re-locate Veeam Agent backup '$($target.BackupName)'."
            continue
        }

        $allPoints = $null
        try { $allPoints = Get-AgentRestorePoints -Backup $backupObj } catch { }
        ...
        if (-not $pointsToRemove -or $pointsToRemove.Count -eq 0) {
            $pointsToRemove = $allPoints | Where-Object {
                try { $_.Name -ieq $target.ComputerName } catch { $false }
            }
        }

        if (-not $pointsToRemove -or $pointsToRemove.Count -eq 0) {
            Write-Warning "No matching restore points found for '$($target.ComputerName)' in backup '$($target.BackupName)'."
            continue
        }
        ...
        if (-not $pointsToRemove -or $pointsToRemove.Count -eq 0) {
            Write-Warning "No stale restore points exist for '$($target.ComputerName)' before the cutoff date."
            continue
        }

        Write-Host "`nThe following Veeam Agent restore points will be DELETED:" -ForegroundColor Yellow
        ...
        try {
            ...
            Write-Host ("Deleted {0} restore point(s) for '{1}' from backup '{2}'." -f `
                $pointsToRemove.Count, $target.ComputerName, $target.BackupName) -ForegroundColor Green

            $remaining = @(Import-Csv -Path $CsvPathCleanup | Where-Object {
                -not (
                    $_.BackupName    -eq $target.BackupName -and
                    $_.ComputerName  -eq $target.ComputerName -and
                    "$($_.ObjectId)" -eq $targetObjectId
                )
            })

            $remaining | Export-Csv -Path $CsvPathCleanup -NoTypeInformation -Encoding UTF8
            Write-Host "Updated Veeam Agent CSV: $CsvPathCleanup" -ForegroundColor DarkGray

        } catch {
            Write-Warning "Failed to delete restore points for '$($target.ComputerName)': $_"
        }
    }
